...
app.post('/api/rooms/:id/join', (req, res) => {
  ...
  res.json({ room: { id: req.params.id, name: room.name, expiresAt: room.expires_at }, messages: (messages[req.params.id] || []).slice(-200) });
});
...
:root{--bg:#faf5ef;--bg2:#ffffff;--text:#1a1a1a;--text2:#6b6158;--orange:#FF6B35;--orange2:#FF8C42;--orange3:#FFA94D;--border:#e8ddd0;--msg-other:#ffffff;--msg-mine:#FFF3EB;--shadow-sm:0 1px 3px rgba(0,0,0,0.04);--shadow:0 4px 24px rgba(0,0,0,0.06);--shadow-lg:0 12px 40px rgba(0,0,0,0.1);--radius:24px;--radius-sm:14px;--radius-xs:10px;--font:'Inter',system-ui,sans-serif;--transition:0.25s cubic-bezier(0.4,0,0.2,1)}
...
@media(min-width:640px){.chat-container{height:calc(100vh - 48px);max-height:750px;border-radius:var(--radius)}.msg{max-width:60%}}
@media(max-width:480px){.app-container{padding:12px}.card{padding:20px}.card-header h1{font-size:22px}.msg{max-width:85%}.chat-header{padding:12px 14px}.chat-input{padding:10px 14px}}
</style>
</head>
<body>
<div id="screenHome" class="app-container"><div class="card"><div class="card-header"><div class="logo-3d"><div class="logo-cube"><div class="logo-face"></div><div class="logo-face"></div><div class="logo-face"></div><div class="logo-face"></div><div class="logo-face"></div><div class="logo-face"></div></div></div><h1>Zal</h1><p data-i18n="tagline">Временные комнаты для быстрых обсуждений. Создай комнату, пригласи друзей, через 24 часа всё исчезнет.</p></div><div class="row gap" style="justify-content:center"><button class="btn btn-primary" onclick="showCreate()" data-i18n="createRoom">Создать комнату</button><input type="text" id="joinId" placeholder="Код комнаты" maxlength="8" style="width:130px;text-align:center"><button class="btn btn-secondary" onclick="joinRoom()" data-i18n="join">Войти</button></div><div class="row gap" style="justify-content:center;margin-top:14px"><button class="btn btn-sm btn-secondary" onclick="toggleTheme()" data-i18n="theme">🌓 Тема</button><select id="langSelect" onchange="setLang(this.value)" style="padding:9px 14px;border-radius:10px;border:1.5px solid var(--border);background:var(--bg2);color:var(--text);font-family:var(--font);font-size:13px;cursor:pointer"><option value="ru">Русский</option><option value="en">English</option></select></div></div></div>
<div id="screenCreate" class="app-container hidden"><div class="card"><div class="card-header"><h1 data-i18n="newRoom">Новая комната</h1></div><label data-i18n="roomName">Название (необязательно)</label><input type="text" id="roomName" placeholder="Вечерний движ" maxlength="30"><label data-i18n="roomPass" style="margin-top:14px">Пароль (необязательно)</label><input type="text" id="roomPass" placeholder="••••••" maxlength="20"><div class="captcha-box"><div id="captchaSvg"></div><input type="text" id="captchaInput" placeholder="Код" maxlength="6" style="width:100px;text-align:center"></div><div class="row gap"><button class="btn btn-primary" onclick="createRoom()" data-i18n="create">Создать</button><button class="btn btn-secondary" onclick="showHome()" data-i18n="back">Назад</button></div></div></div>
<div id="screenChat" class="app-container hidden"><div class="chat-container"><div class="chat-header"><h2 id="chatTitle">Комната</h2><span class="badge" id="onlineCount">1</span><div class="chat-header-actions"><button class="btn btn-sm btn-secondary" onclick="openSettings()">⚙️</button><button class="btn btn-sm btn-secondary" onclick="leaveRoom()" data-i18n="leave">Выйти</button></div></div><div class="typing hidden" id="typingIndicator"><div class="typing-dots"><span></span><span></span><span></span></div><span id="typingText"></span></div><div class="chat-msgs" id="chatMessages"></div><div class="chat-input"><div class="row"><input type="text" id="msgInput" placeholder="Сообщение..." data-i18n-placeholder="msgPlaceholder" autocomplete="off"><button class="btn btn-sm btn-secondary btn-icon" onclick="sendLocation()" title="Гео">📍</button><button class="btn btn-sm btn-primary btn-icon" onclick="sendMsg()" title="Отправить">➤</button></div></div></div></div>
<div id="modalSettings" class="modal-overlay hidden" onclick="if(event.target===this)closeSettings()"><div class="modal"><h3 data-i18n="settings">Настройки</h3><label data-i18n="theme">Тема</label><select id="themeSelect" onchange="setTheme(this.value)" style="width:100%;padding:10px;border-radius:10px;border:1.5px solid var(--border);background:var(--bg2);color:var(--text);font-family:var(--font);font-size:14px;cursor:pointer"><option value="light" data-i18n="themeLight">Светлая</option><option value="dark" data-i18n="themeDark">Тёмная</option></select><label style="margin-top:14px" data-i18n="language">Язык</label><select id="settingsLangSelect" onchange="setLang(this.value)" style="width:100%;padding:10px;border-radius:10px;border:1.5px solid var(--border);background:var(--bg2);color:var(--text);font-family:var(--font);font-size:14px;cursor:pointer"><option value="ru">Русский</option><option value="en">English</option></select><div class="gap"><button class="btn btn-primary" onclick="closeSettings()" data-i18n="close" style="width:100%">Закрыть</button></div></div></div>
<div id="modalPassword" class="modal-overlay hidden" onclick="if(event.target===this)closePassword()"><div class="modal"><h3 data-i18n="passRequired">Требуется пароль</h3><input type="password" id="passwordInput" placeholder="Пароль"><div class="row gap"><button class="btn btn-primary" onclick="submitPassword()" data-i18n="enter">Войти</button><button class="btn btn-secondary" onclick="closePassword()" data-i18n="cancel">Отмена</button></div></div></div>
<script src="/socket.io/socket.io.js"></script>
<script>
const i18n={ru:{tagline:"Временные комнаты для быстрых обсуждений. Создай комнату, пригласи друзей, через 24 часа всё исчезнет.",createRoom:"Создать комнату",join:"Войти",theme:"🌓 Тема",newRoom:"Новая комната",roomName:"Название (необязательно)",roomPass:"Пароль (необязательно)",create:"Создать",back:"Назад",leave:"Выйти",settings:"Настройки",themeLight:"Светлая",themeDark:"Тёмная",language:"Язык",close:"Закрыть",passRequired:"Требуется пароль",enter:"Войти",cancel:"Отмена",msgPlaceholder:"Сообщение...",online:"онлайн",typing:"печатает...",captchaInvalid:"Неверная капча",roomNotFound:"Комната не найдена",wrongPass:"Неверный пароль",location:"Моё местоположение",copied:"Ссылка скопирована!",geoNotSupported:"Геолокация не поддерживается",geoError:"Не удалось получить местоположение"},en:{tagline:"Temporary rooms. Create a room, invite friends, everything disappears in 24 hours.",createRoom:"Create Room",join:"Join",theme:"🌓 Theme",newRoom:"New Room",roomName:"Name (optional)",roomPass:"Password (optional)",create:"Create",back:"Back",leave:"Leave",settings:"Settings",themeLight:"Light",themeDark:"Dark",language:"Language",close:"Close",passRequired:"Password Required",enter:"Enter",cancel:"Cancel",msgPlaceholder:"Message...",online:"online",typing:"typing...",captchaInvalid:"Invalid captcha",roomNotFound:"Room not found",wrongPass:"Wrong password",location:"My location",copied:"Link copied!",geoNotSupported:"Geolocation not supported",geoError:"Could not get location"}};
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    if (!rooms[data.roomId]) { socket.emit('kicked'); return; }
